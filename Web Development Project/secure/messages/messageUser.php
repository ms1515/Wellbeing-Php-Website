...
<body class="bodyBackground">
    ...
    <div class="container-fluid" id="profileContainer">
        ...
        $secondUserId = $_GET['userId'];
        $secondUserName = "";
        $getSecondUserQuery = "SELECT * FROM users WHERE users.id = $secondUserId";
        $getSecondUserResult = $conn->query($getSecondUserQuery);

        if (!$getSecondUserResult) {
            echo "<p class='containerP error'> Error fetching details for recipient user: $conn->error </p>";
        } else {
            $secondUser = $getSecondUserResult->fetch_assoc();
            $secondUserName = $secondUser['firstName'] . " " . $secondUser['lastName'];
        }

        echo "  <div class='row'>
                <div class='col-md-10'>
                <h1 id='h1TitleStyle'>$secondUserName</h1>
                </div>
                </div>";

        $fetchUserMessagesQuery = "SELECT users.firstName, users.email, userMessages.message 
                            FROM userMessages 
                            INNER JOIN users 
                            ON userMessages.user1Id = users.id 
                            WHERE (userMessages.user1Id = $currentUserId AND userMessages.user2Id = $secondUserId)
                            OR (userMessages.user1Id = $secondUserId AND userMessages.user2Id = $currentUserId)
                           ";

        $fetchUserMessagesResult = $conn->query($fetchUserMessagesQuery);

        if (!$fetchUserMessagesResult) {
            echo "<p class='containerP error'> Error Obtaining messages for users: $conn->error </p>";
        } else {
            if ($fetchUserMessagesResult->num_rows == 0) {
                echo "<div class='row profileRow'>
        <div class='col'>
            <div class='card mb-3 cardBlurred'>
            <div class='card-body'>
            <p class='card-text containerP'> You have yet to begin a conversation with $secondUserName </p>
            </div>
            </div>
        </div>
    </div>";
            ...
            while ($message = $fetchUserMessagesResult->fetch_assoc()) {
    ...
    <div class='row profileRow'>
        <div class='col'>
            <div class='card mb-3 cardBlurred'>
                <div class='card-body'>
                    <form class="needs-validation" action=<?php echo "insertUserMessage.php"; ?> method="POST" novalidate>
                        <div class="input-group mb-3 form-group">
                            <input type="text" hidden=true name="recipientUserId" value=<?php echo $secondUserId; ?>>
                            ...
                            <div class="input-group-append">
                                <button class="btn btn-primary approveButton" type="submit" name="insertUserMessageRequest">Send Message</button>
                            </div>
                        </div>
                    </form>
                    <p id="errorMessage">
